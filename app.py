...
# ---------- DEPLOY BUTTON ----------
st.markdown("""
<a href="https://hospital-command-center.streamlit.app"
...
if menu == "Patient Journey":
    ...
    st.bar_chart(patient_df["Department"].value_counts(), color="#00E5FF")

    st.subheader("🦠 Disease Type Count")
    st.bar_chart(patient_df["Disease_Type"].value_counts(), color="#FFD700")

    st.subheader("🚨 Emergency Flag Count")
    st.bar_chart(patient_df["Emergency_Flag"].value_counts(), color="#FF4B4B")
